...
function ThemeCard({ theme }: { theme: typeof themes[0] }) {
  ...
  return (
    <div ref={wrapperRef} className="relative" onMouseEnter={() => tlRef.current?.restart()}>
      {/* Goo bubble circles — top-left */}
      <span
        className="absolute pointer-events-none z-10"
        style={{ top: 0, left: 0, width: '60%', height: '60%', filter: 'url(#goo)', WebkitFilter: 'url(#goo)' }}
      >
        {[0, 1, 2].map(i => (
          <span key={`tl-${i}`} className="card-circle top-left absolute w-[20px] h-[20px] rounded-full bg-white/80"
            style={{ top: '70%', left: '20%' }} />
        ))}
      </span>

      {/* Goo bubble circles — bottom-right */}
      <span
        className="absolute pointer-events-none z-10"
        style={{ bottom: 0, right: 0, width: '60%', height: '60%', filter: 'url(#goo)', WebkitFilter: 'url(#goo)' }}
      >
        {[0, 1, 2].map(i => (
          <span key={`br-${i}`} className="card-circle bottom-right absolute w-[20px] h-[20px] rounded-full bg-white/80"
