...
function ThemeCard({ theme }: { theme: typeof themes[0] }) {
  ...
  function init() {
    ...
    tlAnim.to(tl, 1.1, { x: -30, y: -30, ease: SlowMo.ease.config(0.1, 0.7, false) })
    tlAnim.to(tl[0], 0.1, { scale: 0.2, x: '+=6',  y: '-=3' })
    tlAnim.to(tl[1], 0.1, { scale: 0.8, x: '-=7',  y: '-=3' }, '-=0.1')
    tlAnim.to(tl[2], 0.1, { scale: 0.2, x: '-=15', y: '+=6' }, '-=0.2')
    tlAnim.to(tl[0], 1, { scale: 0, x: '-=5',  y: '-=15', opacity: 0 })
    tlAnim.to(tl[1], 1, { scale: 0.4, x: '-=7',  y: '-=7',  opacity: 0 }, '-=1')
  ...
  return (
    <div ref={wrapperRef} className="relative" onMouseEnter={() => {
      ...
      {/* z-0: circles sit behind the card */}
      <span
